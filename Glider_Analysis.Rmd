...
contours_df_WA_shallow <- ggplot(contours_df, aes(x = sal, y = temp_num)) +
  ...
  geom_contour(aes(z = spice), col="seagreen3", binwidth = 0.5) +
  theme_DOM() +
  labs(y ="Temperature (C)", x="Salinity (ppt)", color = "Dissolved Oxygen (umol/kg)") +
  metR::geom_text_contour(aes(z = dens), stroke = 0.15, skip=0)+
  metR::geom_text_contour(aes(z = spice), stroke = 0.15, skip=0)+
...
```{r interpolation of depth, echo=FALSE, warning=FALSE, tidy = TRUE, message=FALSE}

est_var_by_dist <- function(df, target_var, target_dist, target_depth) {
  data_for_dist <- df %>%
    filter(distance_km == target_dist) %>%
    arrange(depth)
  var <- target_var
...
glider_WA_shallow <- glider_WA %>% 
                  filter(ID == "USA_WA_shallow")  %>% 
                  mutate(distance_km = round(distance_km, 4)) 

# est_var_by_dist(glider_WA_shallow, "temperature", 66.7397, c(6.317120, 8, 9.341731, 10))

temp_grid <- as.data.frame(matrix(data=NA, ncol=2, nrow=0))
colnames(temp_grid) <- c("distance_km", "depth")

for(i in 1:length(unique(glider_WA_shallow$distance_km))){

  xx <- glider_WA_shallow %>%
    filter(distance_km == unique(glider_WA_shallow$distance_km)[i]) %>%
    dplyr::select(distance_km, depth) %>%
    filter(depth != "NaN")
  xx_depths <- xx$depth
  
  xx_max <- round(max(xx$depth), 1)
  xx_min <- round(min(xx$depth), 1)
  
  xx_seq <- seq(xx_min, xx_max, by = 0.1)
  
  depths_above <- glider_WA_shallow %>%
    dplyr::select(depth) %>%
    filter(depth <= xx_max)
  
  depths_all <- unique(c(xx_seq, xx_depths, depths_above$depth))

  yy <- as.data.frame(matrix(data=NA, ncol=2, nrow = length(depths_all)))
  colnames(yy) <- c("distance_km", "depth")
  yy$distance_km <- unique(glider_WA_shallow$distance_km)[i]
  yy$depth <- depths_all
  
  temp_grid <- rbind(temp_grid, yy)
  
}

temp_interp_depth <- temp_grid %>%
  group_by(distance_km) %>%
  mutate(temp = est_var_by_dist(glider_WA_shallow, "temperature", distance_km[1], depth)) %>%
  drop_na(temp)

# ggplot(temp_interp_depth, aes(y=-depth, x=distance_km)) +
#   geom_point(aes(col = temp)) +
#   theme_DOM() +
#   scale_color_viridis() +
#   labs(x ="Distance from shore (km)", y="Depth (m)", color = "Temp")

# ggplot(glider_WA_shallow, aes(y=-depth, x=distance_km)) +
#   geom_point(aes(col = temperature)) + 
#   theme_DOM() +
#   scale_color_viridis() + 
#   labs(x ="Distance from shore (km)", y="Depth (m)", color = "Temp") 
...
```{r interpolating across distance, echo=FALSE, warning=FALSE, tidy = TRUE, message=FALSE}

max_depths_WA_shallow <- glider_WA_shallow %>%
  group_by(distance_km) %>%
  filter(depth == max(depth))

max_depths_WA_shallow <- as.data.frame(matrix(data=NA, ncol=2, nrow = length(unique(glider_WA_shallow$distance_km))))
colnames(max_depths_WA_shallow) <- c("distance_km", "max_depth")
max_depths_WA_shallow$distance_km <- unique(glider_WA_shallow$distance_km)

for(i in 1:nrow(max_depths_WA_shallow)){

  xx <- glider_WA_shallow %>%
    filter(distance_km == max_depths_WA_shallow$distance_km[i]) %>%
    filter(depth != "NaN")
  max_depths_WA_shallow$max_depth[i] <- max(xx$depth)
}

max_depths_WA_shallow <- max_depths_WA_shallow %>%
  filter(distance_km < 36 | (distance_km > 36 & max_depth > 64))

max_WA_shallow_dist <- round(max(max_depths_WA_shallow$distance_km), 1)
min_WA_shallow_dist <- round(min(max_depths_WA_shallow$distance_km), 1)

smooth_vals = predict(loess(max_depth~distance_km,max_depths_WA_shallow), seq(min_WA_shallow_dist, max_WA_shallow_dist, by=1))

maxdepth_dist_smooth <- data.frame(distance_km = seq(min_WA_shallow_dist, max_WA_shallow_dist, by=1), smooth = smooth_vals)
...
temp_grid_2 <- as.data.frame(matrix(data=NA, ncol=2, nrow=0))
colnames(temp_grid_2) <- c("distance_km", "depth")

for(i in 1:length(unique(temp_interp_depth$depth))){
  
  xx <- temp_interp_depth %>%
    filter(depth == unique(temp_interp_depth$depth)[i])
  xx_dists <- xx$distance_km
  
  xx_max <- round(max(xx$distance_km), 1)
  xx_min <- round(min(xx$distance_km), 1)
  
  xx_seq <- maxdepth_dist_smooth %>%
    filter(distance_km >= xx_min & distance_km <= xx_max)

  yy <- as.data.frame(matrix(data=NA, ncol=2, nrow = nrow(xx_seq)))
  colnames(yy) <- c("distance_km", "depth")
  yy$depth <- unique(temp_interp_depth$depth)[i]
  yy$distance_km <- xx_seq$distance_km
  
  temp_grid_2 <- rbind(temp_grid_2, yy)
...
temp_raster <- temp_grid_2 %>%
  group_by(depth) %>%
  mutate(temp = estimate_var_by_depth(temp_grid, "temperature", depth[1], distance_km)) %>%
  drop_na(temp)

ggplot(temp_raster, aes(distance_km, depth, fill = temp)) +
  geom_raster() +
  scale_y_reverse() +
