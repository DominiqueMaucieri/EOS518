...
# ggplot(interpdepth_WA_deep, aes(y=-depth, x=distance_km)) +
#   geom_point(aes(col = density)) +
#   theme_DOM() +
#   scale_color_viridis() +
#   labs(x ="Distance from shore (km)", y="Depth (m)", color = "density")
...
# interp_grid_dist_WA_deep <- read.csv("interp_grid_dist_WA_deep.csv")
...
for(i in 1:nrow(dens_cont_WA_shallow)){
  ...
  if(xx_den_26 != xx_max_depth){
    dens_cont_WA_shallow$den_26[i] <- xx_den_26
  } else {
    dens_cont_WA_shallow$den_26[i] <- NA
  }
    
   xx_den_26.2 <- xx[which.min(abs(xx$density - 26.2)),]$depth
  if(xx_den_26.2 != xx_max_depth){
    dens_cont_WA_shallow$den_26.2[i] <- xx_den_26.2
  } else {
    dens_cont_WA_shallow$den_26.2[i] <- NA
  }
  
   xx_den_26.4 <- xx[which.min(abs(xx$density - 26.4)),]$depth
  if(xx_den_26.4 != xx_max_depth){
    dens_cont_WA_shallow$den_26.4[i] <- xx_den_26.4
  } else {
    dens_cont_WA_shallow$den_26.4[i] <- NA
  }
   
   xx_den_26.6 <- xx[which.min(abs(xx$density - 26.6)),]$depth
  if(xx_den_26.6 != xx_max_depth){
    dens_cont_WA_shallow$den_26.6[i] <- xx_den_26.6
  } else {
    dens_cont_WA_shallow$den_26.6[i] <- NA
  }
   
   xx_den_26.8 <- xx[which.min(abs(xx$density - 26.8)),]$depth
  if(xx_den_26.8 != xx_max_depth){
    dens_cont_WA_shallow$den_26.8[i] <- xx_den_26.8
  } else {
    dens_cont_WA_shallow$den_26.8[i] <- NA
  }
   
   xx_den_27 <- xx[which.min(abs(xx$density - 27)),]$depth
  if(xx_den_27 != xx_max_depth){
    dens_cont_WA_shallow$den_27[i] <- xx_den_27
  } else {
    dens_cont_WA_shallow$den_27[i] <- NA
  }
}

dens_cont_WA_shallow_27 <- dens_cont_WA_shallow %>%
  filter(distance_km > 35) %>%
  dplyr::select(distance_km, den_27) %>%
  drop_na(den_27)

dens_cont_WA_shallow_26.8 <- dens_cont_WA_shallow %>%
  filter(distance_km > 25) %>%
  dplyr::select(distance_km, den_26.8) %>%
  drop_na(den_26.8)

dens_cont_WA_shallow_26.6 <- dens_cont_WA_shallow %>%
...
ggplot(raster_dens_WA_shallow, aes(distance_km, depth, fill = density)) +
  ...
  theme_DOM() +
  labs(y="Depth (m)", x = "Distance (km)", fill = "Density (kg/m3)") +
  geom_smooth(dens_cont_WA_shallow, mapping = aes(x=distance_km, y=den_26, fill=NULL), col="red", span = 0.1) +
  geom_smooth(dens_cont_WA_shallow, mapping = aes(x=distance_km, y=den_26.2, fill=NULL), col="red", span = 0.1) +
  geom_smooth(dens_cont_WA_shallow, mapping = aes(x=distance_km, y=den_26.4, fill=NULL), col="red", span = 0.1) +
  geom_smooth(dens_cont_WA_shallow_26.6, mapping = aes(x=distance_km, y=den_26.6, fill=NULL), col="red", span = 0.1) +
  geom_smooth(dens_cont_WA_shallow_26.8, mapping = aes(x=distance_km, y=den_26.8, fill=NULL), col="red", span = 0.1) +
  geom_smooth(dens_cont_WA_shallow_27, mapping = aes(x=distance_km, y=den_27, fill=NULL), col="red", span = 0.1)
...
long_dens_cont_WA_shallow <- dens_cont_WA_shallow %>%
  dplyr::select(distance_km, den_26, den_26.2, den_26.4) %>%
  pivot_longer(cols = c("den_26", "den_26.2", "den_26.4"), names_to = "isopycnal", values_to = "depth")

long_dens_cont_WA_shallow_26.6 <- dens_cont_WA_shallow_26.6 %>%
  pivot_longer(cols = c("den_26.6"), names_to = "isopycnal", values_to = "depth")

long_dens_cont_WA_shallow_26.8 <- dens_cont_WA_shallow_26.8 %>%
  pivot_longer(cols = c("den_26.8"), names_to = "isopycnal", values_to = "depth")

long_dens_cont_WA_shallow_27 <- dens_cont_WA_shallow_27 %>%
  pivot_longer(cols = c("den_27"), names_to = "isopycnal", values_to = "depth")

long_dens_cont_WA_shallow <- rbind(long_dens_cont_WA_shallow, long_dens_cont_WA_shallow_26.6, long_dens_cont_WA_shallow_26.8, long_dens_cont_WA_shallow_27)
