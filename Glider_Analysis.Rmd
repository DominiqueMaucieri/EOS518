...
ggplot(glider, aes(x=depth, y=dissolved_oxygen)) +
  geom_point(aes(col = date)) +
  scale_color_viridis(name='Date', 
                      trans = "date", 
                      labels = label_date(format = "%Y-%m-%d") ) + 
  ...
  facet_wrap(vars(ID), scales = "free_x") + 
  labs(x ="Depth", y="Dissolved Oxygen (umol/kg)") +
...
```{r vancouver island map, warning=FALSE, tidy = TRUE, message=FALSE}

dfo_k999_nc <- nc_open(here::here("", "dfo_k999.nc"))
names(dfo_k999_nc$var)

dfo_k999 <- as.data.frame(matrix(data=NA, nrow=7983, ncol=11))
colnames(dfo_k999) <- c("Time", "latitude", "longitude", "chlorophyll", "salinity", "depth", "oxygen_concentration", "temperature", "backscatter_700", "cdom", "ID")
...
dfo_k999$oxygen_concentration <- ncvar_get(dfo_k999_nc, varid = c("oxygen_concentration"))
dfo_k999$temperature <- ncvar_get(dfo_k999_nc, varid = c("temperature"))
dfo_k999$backscatter_700 <- ncvar_get(dfo_k999_nc, varid = c("backscatter_700"))
