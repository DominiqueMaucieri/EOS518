...
```{r profile distance from shore wiht density, echo=FALSE, warning=FALSE, tidy = TRUE, message=FALSE}

comb_glider$ID <- factor(comb_glider$ID, levels = c("CAN_VI_deep", "CAN_QC_shallow", "USA_WA_deep",  "USA_WA_shallow"))

ggplot(comb_glider %>% drop_na(density), aes(y=-depth, x= distance_km)) +
  geom_point(aes(col = (density -1000))) + 
  theme_DOM() +
  scale_color_viridis() +
...
tempvDO_plot <- ggplot(comb_glider, aes(y=dissolved_oxygen, x=temperature, col = ID)) +
  ...
  scale_color_manual(values = c("chocolate1", "orangered3", "mediumpurple1", "blue2")) +
...
ggplot(contours_df, aes(x = sal, y = temp_num)) +
  ...
  theme_DOM() +
  labs(y ="Temperature (C)", x="Salinity (ppt)", color = "ID") +
...
```{r deep contours, echo=FALSE, warning=FALSE, tidy = TRUE, message=FALSE}


comb_glider_deep <- comb_glider %>%
  filter(ID == "CAN_VI_deep" | ID == "USA_WA_deep")
...
ggplot(contours_df, aes(x = sal, y = temp_num)) +
  ...
  metR::geom_text_contour(aes(z = dens), stroke = 0.15, skip=0)+
  metR::geom_text_contour(aes(z = spice), stroke = 0.15, skip=0) +
  geom_point(comb_glider_deep, mapping = aes(y=temperature, x=salinity, col = ID), alpha = 0.3) +
  scale_color_manual(values = c("orangered3", "blue2")) +
  xlim(31, 34.5) + 
  ylim(3,17)
...
comb_glider_shallow <- comb_glider %>%
  filter(ID == "CAN_QC_shallow" | ID == "USA_WA_shallow")
...
ggplot(contours_df, aes(x = sal, y = temp_num)) +
  ...
  metR::geom_text_contour(aes(z = dens), stroke = 0.15, skip=0)+
  metR::geom_text_contour(aes(z = spice), stroke = 0.15, skip=0) +
  geom_point(comb_glider_shallow, mapping = aes(y=temperature, x=salinity, col = ID), alpha = 0.3) +
  scale_color_manual(values = c("chocolate1", "mediumpurple1")) +
  xlim(31, 34.5) + 
  ylim(3,17)
...
```{r density contour WA, echo=FALSE, warning=FALSE, tidy = TRUE, message=FALSE}
...
```
```{r density contour QC, echo=FALSE, warning=FALSE, tidy = TRUE, message=FALSE}
