...
ggplot(glider_320, aes(x=longitude, y = latitude)) +
  ...
  theme_DOM() + 
  labs(x ="Longitude", y="Latitude", title = "USA_OR_shallow")
...
ggplot(glider_383, aes(x=longitude, y = latitude)) +
  ...
  theme_DOM() + 
  ylim(c(46.75, 47.25)) + 
  labs(x ="Longitude", y="Latitude", title = "USA_OR_deep")
...
simple_map_OR <- ggplot(data = world) +
  ...
  scale_color_manual(values = c("lightslateblue", "darkorchid")) +
...
glider_OR$ID <- factor(glider_OR$ID, levels = c("USA_OR_shallow", "USA_OR_deep"))
...
# Both Canadian and Oregon Gliders
...
comb_glider$ID <- factor(comb_glider$ID, levels = c("CAN_QC_shallow", "CAN_VI_deep", "USA_OR_shallow", "USA_OR_deep"))
...
ggplot(comb_glider, aes(y=-depth, x= distance_km)) +
  ...
  scale_color_viridis() +
  facet_wrap(vars(ID), scales = "free_x", ncol=2) + 
  labs(x ="Distance from shore (km)", y="Depth (m)", color = "Salinity")
...
ggplot(comb_glider, aes(y=-depth, x= distance_km)) +
  ...
  scale_color_viridis() +
  facet_wrap(vars(ID), scales = "free_x", ncol=2) + 
  labs(x ="Distance from shore (km)", y="Depth (m)", color = "Chl")

```


```{r profile distance from shore wiht cdom, echo=FALSE, warning=FALSE, tidy = TRUE, message=FALSE, fig.height=8, fig.width=6}
...
ggplot(comb_glider, aes(y=-depth, x= distance_km)) +
  ...
  scale_color_viridis() +
  facet_wrap(vars(ID), scales = "free_x", ncol=2) + 
  labs(x ="Distance from shore (km)", y="Depth (m)", color = "CDOM")
